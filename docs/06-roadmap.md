...
barely started. A naive checkbox count says 62% complete; weighted by effort and
risk it is closer to **40%**, because the one item that gates everything —
a training run that demonstrably learns — is not done.
...
| 100 tests across core, services and the research package | done |
...
| Item | Evidence |
|---|---|
| Truncation fix | mid-sentence turns down to ~5% |
| Dangling-promise trim | 19% of turns ended in `:` and invited completion |
...
| Scaled config | `G=8`, 20 epochs, 320 dialogues |

### Not done

| Item | Why it matters |
|---|---|
| **A scaled run that shows learning** | The gate for the entire project. 24 dialogues proved nothing; 320 either shows a trend or shows there isn't one. |
| **Role inversion resolved** | Tutor writes code in 14% of turns and asks a question in only 37%; student writes code in 18%. Countermeasures shipped but unverified. |
...
second T4 is the obvious next lever and is mostly free.

---

## Phase 3 — Indic / code-mixed 🟡 **scaffolding only**

The project's actual differentiator, and the least real so far.
...
- `services/asr` boundary, contract, and health check
...
| ASR that actually transcribes | The service returns `501`; `whisper-large-v3` is mediocre on code-mixed Indic anyway. |
| Code-mixed evaluation set | No way to measure whether Indic performance is good or bad. |
| Indic-aware pedagogy checks | The 5 checks are English-shaped; needs verifying across scripts. |

**Note:** continued pretraining is where catastrophic forgetting risk is
genuinely high — unlike the LoRA stage, which freezes the base weights.
...
1. **Analyse the scaled run** when it lands — does `ped` trend, and did the
   role-inversion fixes take?
2. **Student → 1.5B on the second GPU** if inversion persists.
